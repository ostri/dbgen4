...
public:
  ...
  [[nodiscard]] vec_str_t
  files() const; //< fetch the list of gsql file to be processed
  [[nodiscard]] db_type_enum
  db_type() const; //< fetch the database type to which to generate code
  [[nodiscard]] str_t
  db_name() const; //< fetch the database name that holds the table definitions
