...
namespace dbgen4
{
  ...
  public:
    sql_col_def() = default;
    sql_col_def(uint16_t pos, const cstr_t& name, rtl::sql_type type, bool nullable)
    : position_(pos)
    , name_(name)
    , type_(type)
    , nullable_(nullable)
    ...
    [[nodiscard]] rtl::sql_type type() const;
  ...
  private:
    ...
    rtl::sql_type type_{};         ///< data type of the column/parameter
