 ...
 *
 * db2-only: the psql runtime's query<>::execute() marshals and sends only
 * row 0 of the parameter buffer, so there is no multi-row batch execute to
 * exercise there yet. Split out of test_crud.cpp so the single-row round
 * trip in that file can be shared between both backends while this stays
 * ODBC-specific (rtl::odbc_error, SQL_PARAM_ERROR, per-row status arrays).
 */
#include "crud.hpp"
#include "db2_rtl.hpp"
#include "rtl.hpp"
