...
namespace rtl
{
  ...
  : ret_(r)
  {
    ...
    auto          dr   = SQLGetDiagRec(type, h, rec, state, &native_error_, msg, sizeof(msg), &msg_len); // NOLINT
    if (SQL_SUCCEEDED(dr))
    {
      sql_state = std::string(reinterpret_cast<char*>(state), 5); // NOLINT
      message   = sqlchar_to_utf8(msg, msg_len);                  // NOLINT
    }
    else
    {
      message   = "ODBC diagnostic retrieval failed";
      sql_state = "HY000";
    }
  }
