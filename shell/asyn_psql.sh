#!/usr/bin/env bash
# asyn_psql.sh - run the sync-vs-async_db insert throughput benchmark
# (test_bench_async.cpp) against the PostgreSQL test database.
#
# Run from wherever the binary lives: build/release, build/debug or
# build/profile (e.g. `../../shell/asyn_psql.sh` from build/release).
#
...
LOG_DIR="/tmp/dbgen4-bench-psql"
mkdir -p "${LOG_DIR}"
LOG_CONFIG_FILE="$(mktemp /tmp/dbgen4_log_bench_psql.XXXXXX.conf)"
